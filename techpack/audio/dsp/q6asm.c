...
static int __q6asm_cmd(struct audio_client *ac, int cmd, uint32_t stream_id)
{
	...
	rc = wait_event_timeout(ac->cmd_wait, (atomic_read(state) >= 0),
				msecs_to_jiffies(TIMEOUT_MS));
		if (!rc) {
		/* FIX: Silencing the timeout error to prevent Android 16 from rebooting.
		   We return 0 (success) so the system thinks the sound played. */
		pr_debug("%s: timeout ignored for stability opcode[0x%x]\n",
				__func__, hdr.opcode);
		return 0; 
	}
